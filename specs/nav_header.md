...
<Layout>          {/* renders <Header/> once, above the routed outlet */}
  <Routes>
    ...
    <Route path="/termites"      element={<Flat/>} />     {/* was the old landing; termites → here */}
...
`[ Thank God for Mercy Land wordmark ]  ······  [ termites ]  [ listen ]  [ merch ]`

- **Wordmark → world root (`/`).** This is the utility layer's way back into the world. On
  non-scene routes (`/termites`, `/void`, `/chat`) it is the **only** guaranteed return, so it must
...
- **termites · listen · merch** — the three utility items.
...
|---|---|---|---|---|
| *(wordmark)* Thank God for Mercy Land | `/` (world root) | internal | no | return to the world |
| termites | `/termites` | internal | no | the former landing; labelled `info` until 2026-07-25 |
| listen | Bandcamp `/music` | external | yes | RESOLVED: external streaming, no `/listen` route |
| merch | Bandcamp `/merch` | external | yes | existing link |
...
const navItems: NavItem[] = [
  ...
  { label: 'listen',   kind: 'external', url: LISTEN_URL }, // thankgodformercyland.bandcamp.com/music
  { label: 'merch',    kind: 'external', url: MERCH_URL },  // thankgodformercyland.bandcamp.com/merch
];
...
- **Identical on every route.** Same component, same items, same behavior on the house world,
  `/termites`, `/void`, `/chat`. Only the *skin* is designer-fill (tokens); structure never varies.
- **Always reachable.**
- **Active state.** The item matching the current route gets active styling (e.g. `termites` is
  active on `/termites`). External items never show active.
...
- **On scenes:** the header sits above the diegetic hotspots and universal returns. It's the
  utility escape hatch — streaming, merch, the old landing — that never has to compromise the scene's mood.
- **On non-scene routes (`/termites`, `/void`, `/chat`):** the header is the only persistent chrome.
...
- [ ] Confirm the shared header renders on `/termites` via the layout (nothing flat-specific added).
- [ ] Verify the wordmark returns to the world from `/termites` (no dead end).
- [ ] Confirm `termites` shows active state while on `/termites`.
...
| `--header-wordmark-size` | wordmark size (fluid — the wordmark is long) | `clamp(0.95rem, 4.2vw, 1.35rem)` |
...
1. ~~**`listen` destination**~~ — RESOLVED 2026-07-25: a single external streaming link
   (`thankgodformercyland.bandcamp.com/music`), `kind: 'external'`. No `/listen` route.
2. **Email in the header?** — signup is currently one hop from start (mailbox scene). Does the
   header also get a 4th item / put "join the list" under `termites`, or stay wordmark + three?
