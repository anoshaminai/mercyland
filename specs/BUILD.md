...
3. **Generic components**, per spec:
   ...
   - `Scene` — renders photo in the frame; `pannable` vs `static` (spec §1, §5, graph Issue 6);
     pannable = wider-than-viewport portrait + edge indicators + **focus-driven panning**
   ...
   - `Header` — own component; `info · listen · merch`; collapses to a toggle on mobile (spec §3).
...
5. **Routes** — `/void`, `/chat` (hotspot `enter` targets), `/flat` (header "info" — this is the
   old landing, formerly "/termites"). Ensure the **Render rewrite** `/*` → `/index.html` (Rewrite)
...
- **Issue 3** — old landing is `/flat`; `/termites` retired for now.
...
- **Portrait crops** — `imageTall` for each `pannable` scene, authored **wider than the viewport**.
- **Streaming** — `song01` URL; header "listen" destination.
